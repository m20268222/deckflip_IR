...
(function(){
  ...
  opts.forEach(function(o,i){
    o.addEventListener('click',function(){
      ...
      if(prev===String(i)){orbs(1,i);return}
      try{localStorage.setItem(myKey(),String(i))}catch(_){}
      counts[i]++;if(prev!==null&&counts[prev|0]>0)counts[prev|0]--;
      ...
      if(state)state.textContent=prev===null?'고마워요, 무대가 방금 움직였어요':'마음을 바꾸셨네요. 바로 반영했어요';
    });
  });
...
/* ── S9 피날레: 재진입마다 수렴 ── */
(function(){
  var sc=$('finale'),conv=$('conv');if(!sc)return;
  var seeded=false;
  function seed(){
    if(seeded)return;seeded=true;
    var spots=[[8,18],[24,72],[16,44],[38,12],[46,82],[60,20],[72,66],[84,30],[90,74],[66,42],[30,30],[80,10]];
    spots.forEach(function(s,i){
      var el=document.createElement('i');
      el.style.setProperty('--x',s[0]+'%');el.style.setProperty('--y',s[1]+'%');
      el.style.setProperty('--w',(46+(i*13)%50)+'px');
      el.style.setProperty('--rz',((i%2?1:-1)*(120+i*24))+'deg');
      el.style.transitionDelay=(i*0.07)+'s';
      conv.appendChild(el);
    });
  }
  function pull(){seed();sc.classList.remove('pull');
    requestAnimationFrame(function(){requestAnimationFrame(function(){
      setTimeout(function(){sc.classList.add('pull')},350)})});
  }
  sc.addEventListener('scene:in',pull);
  sc.__pull=pull;
})();
...
(function(){
  ...
  var SEQ=[
    ...
    {id:'setlist',dur:7000,cap:'오늘 무대는 여섯 막이에요. 막마다 종이는 못 하는 걸 하나씩 보여드릴게요.'},
    ...
    {id:'paper',dur:11000,cap:'숫자가 자라고, 깨진 곳은 알아서 고쳐요. 이 검사는 멈추지 않고 계속 돌아요.'},
    {id:'voice',dur:10000,cap:'이 투표는 진짜예요. 옆자리 폰으로 찍어도, 이 화면에서 눌러도 무대가 바로 대답해요.'},
    {id:'recompile',dur:9500,cap:'복도에서 90초밖에 없다면요? 같은 이야기가 자리에 맞게 다시 짜여요.',
      fn:function(){var r=$('recompile');if(r&&r.__mode){setTimeout(function(){r.__mode('hall')},2400);
        setTimeout(function(){r.__mode('main')},7000)}}},
    {id:'depth',dur:8500,cap:'짧게 듣고 싶은 분에게는 결론만, 깊게 듣고 싶은 분에게는 전부요.',
      fn:function(){var d=$('depth');if(d&&d.__dial){setTimeout(function(){d.__dial('deep')},2400);
        setTimeout(function(){d.__dial('exec')},6800)}}},
    {id:'morph',dur:10000,cap:'발표가 끝나도 이야기는 계속돼요. 랜딩으로, 제안서로, 전시로 옷을 갈아입어요.',
      fn:function(){var m=$('morph');if(m&&m.__set){[1,2,3,0].forEach(function(k,i){
        setTimeout(function(){m.__set(k)},1100+i*2100)})}}},
    {id:'finale',dur:8000,cap:'다음 발표자료는 만들지 마세요. 말해 주시면, 저희가 무대에 올릴게요.'}
  ];
  function startPilot(){
